...
function normalizeConditionRecord(
  record: Partial<ConditionRecord> & { condition_score?: string | number },
) {
  return {
    ...record,
...
export const ConditionService = {
  ...
  get: (id: number | string) =>
    api.get<ConditionRecord>(`/conditions/${id}/`).then((r) => normalizeConditionRecord(r.data)),
  create: (payload: Omit<ConditionRecord, "id" | "asset_name">) =>
    api
      .post<ConditionRecord>("/conditions/", payload)
      .then((r) => normalizeConditionRecord(r.data)),
  update: (id: number | string, payload: Partial<Omit<ConditionRecord, "id" | "asset_name">>) =>
    api
      .put<ConditionRecord>(`/conditions/${id}/`, payload)
      .then((r) => normalizeConditionRecord(r.data)),
